...
%     end
    ...
    err_efftherm = performanceR{2,3} / .5543; %thermal efficiency error compared to book
    err_effprop =performanceR{2,4} / .5914; %propulsive efficiency compared to book
...
function [performance,inputs,state,design,component,M6] = offdesign(inputs,state,design,component,componentR,A16_6,A45_6,M6,M6A_ref,fAB);
    ...
    while check == 0
        ...
        [state,component] = off_burner(state,component);
...
function [state,component] = off_burner(state,component)
state(8,2:3) = state(7,2:3);
state(8,6:12) = state(7,6:12);
