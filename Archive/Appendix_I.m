...
tau_r = state{3,8} / state{2,8};
h0 = state{2,8};
mdot0 = state{2,5};
tau_lambda = ho4/h0;
tau_f = 1 ...
    + ( (1-tau_tL)*etamL* (((1-beta-ep1-ep2)*(1+f)*tau_lambda*tau_tH/tau_r  +  (ep1*tau_tH + ep2)*tau_cL*tau_cH))   -   (1+alpha)*PtoL/(tau_r*etamPL*mdot0*h0)) ...
    / ((tau_cLR - 1)/(tau_fR - 1) + alpha); %where did this come from???
...
pi_f = state{5,2} / state{4,2};
pi_cL = state{6,2} / state{4,2};
...
To16 = To13;
P0 = state{2,2};
pi_r = state{3,2}/state{2,2};
if M6A<1
    pi_d = pi_dmax;
elseif M6A>1 && M6A<5
    pi_d = pi_dmax * (1-.075*((M6A-1)^1.35));
else
    pi_d = pi_dmax * (800/(M6A^4 + 935));
end

Po6 = P0*pi_r*pi_d*pi_cL*pi_cH*pi_b*pi_tHR*pi_tLR;
f45 = f*mdot31 / (mdot31+mdotep1+mdotep2);
[statex] = RGCOMPR(1,To6,f45,M6A); %should be M6. How do we get it?
To6_T6 = statex.T_t_over_T;
Po6_P6 = statex.P_t_over_P;
MFP6 = statex.MFP;
M6 = statex.M;
...
[statex] = RGCOMPR(1,To16,0,1);
ToT16 = statex.T_t_over_T;
PoP16 = statex.P_t_over_P;
MFP16 = statex.MFP;
M16 = statex.M;
...
[statex] = RGCOMPR(3,To16,0,Po16_P16); 
ToT16 = statex.T_t_over_T;
Po16_P16 = statex.P_t_over_P;
MFP16 = statex.MFP;
M16 = statex.M;

T16 = To16/ToT16;
alpha_prime_new = Po16*A16_A6*MFP16*sqrt(To6) ... 
    / (Po6*MFP6*sqrt(To16));
