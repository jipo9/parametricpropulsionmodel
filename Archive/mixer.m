function [state,design,check,M6,alpha_track,M6A_i] = mixer(state,component,design,inputs,A16_6,M6,P6A_ref,alpha_track)
...
if alpha_err > .001 %changed from .001 for speed
    design{2,2} = alpha - a_dampner*(alpha - alpha_i);
    check = 0;
    M6A_i = 0;
else
    [M6A_i,pi_M, state] = mixer_state(state,alpha_prime,A16_6,M16,M6);
    
    alt = inputs{2,2};
    [~, ~, P0, ~] = atmosisa(alt);
    [~,pi_r,pi_d,~,pi_cL,pi_cH,pi_b,~,pi_tH,~,~,pi_tL,~,~,~,~,~] = component{:,2};
    ...
    P6A_err = (P6A_ref - P6A_i)/P6A_ref
    if norm(P6A_err) > .001 %.001
        M6 = M6 - .1*P6A_err;
%         M6 = M6* (1 - .01*P6A_err/norm(P6A_err))
        check = 0;
        alpha_track = 0;
    else
        check = 1;
...
function [M6A,pi,state] = mixer_state(state,alpha_prime,A16_6,M16,M6)
