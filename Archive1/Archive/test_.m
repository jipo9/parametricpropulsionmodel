...
for input = 1:1
    ...
    S = 1.0862*((.453592/3600)/4.44822); %specific fuel consuption[kg/s/N from lbm/(lbf/h)]
    ...
    pi_M_max = .97; %mixer_ pressure ratio
...
for estimating_mixer_props = 1:1
    M6_est = .4;
    [A16_6] = A_mixer_(stateR,componentR,designR,M6_est); %all error comes from errors in turbines
    A16_6 = .2715
...
for ii = 1:size(altitude,2)
    ...
    for jj = 1:size(M,2)
        ...
        mdot(ii,jj) = mdot_;
        alpha_(ii,jj) = design{2,2};
    end
%     plot(M(ii,:),F(ii,:),'linewidth',1.5)
    plot(M(ii,:),S(ii,:),'linewidth',1.5)
%     plot(M(ii,:),alpha_(ii,:),'linewidth',1.5)
    legend('SL','10k ft','20k ft','30k ft','36k ft','40k ft','50k ft','location','best')
    title('Mach Number vs. Specific Fuel Consumption at Various Altitudes')
    xlabel('Mach Number')
    ylabel('Alpha')
    grid('on')
    warning('off','all')
